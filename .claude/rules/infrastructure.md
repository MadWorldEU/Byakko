...
- **Auth:** JWT Bearer via Keycloak (`Authentication:Authority`). Blazor OIDC via `AddOidcAuthentication`; config under `Oidc` in `wwwroot/appsettings.json`. `OidcSettings.GetEditAccountUrl()` returns the Keycloak account page URL. Do **not** add `offline_access` to `DefaultScopes` — it breaks SSO across Portal/Admin by creating per-client offline sessions instead of sharing the Keycloak SSO cookie.
