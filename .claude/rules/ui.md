...
| `Pages/Accounts/AccountDeletionRequests.razor` | `/accounts/deletion-requests` | Paged list of `DeletionRequested`/`DeletionConfirmed` accounts with User ID, status badge, updated timestamp, and inline "Confirm deletion" action with Yes/No prompt; `Administrator` policy |
| `Pages/HostServices/ManualTriggers.razor` | — | Manual cleanup triggers |

Shared services: `IAccountService` (`GetAccountsPendingDeletionAsync`, `ConfirmDeletionAsync`, `CreateMyAccountAsync`, `GetMyAccountAsync`, `RequestDeletionMyAccountAsync`), `IAssetService` (`GetAssetsMetadataAsync`, `DeleteAssetContentAsync`), `IStorageService` (`GetStorageStatisticsAsync`), `IAuditService` (`GetAuditLogsAsync`). Global usings in `_Imports.razor`: `MadWorldEU.Byakko.Services`, `.Storages`, `.Audits`, `.Audits.Summaries`, `.Formatters`. Admin `_Imports.razor` also adds `MadWorldEU.Byakko.Accounts`.
...
| `Pages/Settings.razor` | `/settings` | Account status + GDPR deletion request; auto-creates account on 404; `User` policy |
...
**Settings page:** On load calls `GET /accounts/me`; if 404 automatically calls `POST /accounts/me` then re-fetches. Shows account status badge (`Active` / `Deletion requested`) and a danger-zone card with inline delete confirmation. Settings nav link lives in the user account dropdown (not the top-level nav).
