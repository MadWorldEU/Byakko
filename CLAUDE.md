...
**Upload page** (`Pages/Storage/Upload.razor`, route `/storage/upload`): File picker + optional "Advanced options" section (collapsed by default) containing a password input. On upload, calls `POST /assets` to create metadata then `PUT /assets/{id}/content` (multipart form) with the file and an optional `password` field. Password is sent as `null` when the field is left empty.
